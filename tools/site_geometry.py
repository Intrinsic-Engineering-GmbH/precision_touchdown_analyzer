...
def report(distance: float, mast: float, fps: float, hfov: float) -> None:
    ...
    print(f"Camera {distance:.0f} m from the strip, mast {mast:.1f} m, {fps:.0f} fps, HFOV {hfov:.0f} deg")
    ...
    print(f"  longitudinal error from 1 px vertical error at the frame edge: {lon_from_1px_v:.2f} m")
    print(f"  travel per frame     {TOUCHDOWN_SPEED / fps:6.2f} m  (sub-frame fit ~{TOUCHDOWN_SPEED / fps / 5:.2f} m)")
    for shutter in (500, 1000, 2000):
        blur_mm = TOUCHDOWN_SPEED / shutter * 1000
        print(f"  motion blur 1/{shutter:<5} {blur_mm:6.1f} mm = {blur_mm * px_per_m / 1000:.1f} px")
    for char_cm in (20, 30, 50):
        print(f"  OCR: {char_cm} cm character = {char_cm / 100 * px_per_m:.0f} px "
              f"({'ok' if char_cm / 100 * px_per_m >= 22 else 'too small'})")


def main() -> None:
    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--distance", type=float, default=75.0, help="camera distance from the strip [m]")
    ...
    print(f"  covers {tele_w:.1f} m at {args.distance:.0f} m, {W_PX / tele_w:.0f} px/m "
